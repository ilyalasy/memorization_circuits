# Copied from https://github.com/facebookresearch/llm-transparency-tool/blob/main/llm_transparency_tool/routes/contributions.py
...
@torch.no_grad()
@typechecked
def get_parallel_attention_contributions(
    resid_in: Float[torch.Tensor, "batch pos d_model"],
    decomposed_attn: Float[torch.Tensor, "batch pos key_pos head d_model"],
    mlp_out: Float[torch.Tensor, "batch pos d_model"],
    resid_out: Float[torch.Tensor, "batch pos d_model"],
    distance_norm: int = 1,
) -> Tuple[
    Float[torch.Tensor, "batch pos key_pos head"],
    Float[torch.Tensor, "batch pos"],
    ...
    k = len(parts.shape) - len(resid_out.shape)
                        
    # Flatten the p_ dimensions, get contributions for the list, unflatten.
    flat = parts.flatten(start_dim=0, end_dim=k - 1)
    flat = torch.cat([flat, mlp_out.unsqueeze(0), resid_in.unsqueeze(0)])
    c = get_contributions(flat, resid_out,distance_norm)

    attn_contributions, mlp_contribution, resid_contribution = torch.split(
        c, (flat.shape[0] - 2,1,1)
    )
    attn_contributions = attn_contributions.unflatten(0, parts.shape[0:k])
    attn_contributions = einops.rearrange(
        attn_contributions, "key_pos head batch pos -> batch pos key_pos head"
    )
    return attn_contributions, mlp_contribution.squeeze(0), resid_contribution.squeeze(0)
...
@torch.no_grad()
def apply_threshold_and_renormalize_parallel(
    threshold: float,
    c_attn: torch.Tensor,
    c_ffn: torch.Tensor,
    c_residual: torch.Tensor,
    ...
    block_dims = len(c_attn.shape)
    resid_dims = len(c_residual.shape)
    bound_dims = block_dims - resid_dims
    assert bound_dims >= 0
    assert c_attn.shape[0:resid_dims] == c_residual.shape

    c_attn = c_attn * (c_attn > threshold)
    c_ffn = c_ffn * (c_ffn > threshold)
    c_residual = c_residual * (c_residual > threshold)

    if bound_dims > 0:
        denom = c_residual + c_attn.sum(dim=tuple(range(resid_dims, block_dims))) + c_ffn
    else:
        denom = c_residual + c_attn + c_ffn
    return (
        c_attn / denom.reshape(denom.shape + (1,) * bound_dims),
        c_ffn / denom,
        c_residual / denom,
    )